...
final class DjiDevice: NSObject {
    ...
    func startLiveStream(wifiSsid: String,
                         wifiPassword: String,
                         rtmpUrl: String,
    ...
    func getBatteryPercentage() -> Int? { batteryPercentage }
    ...
    private func stopStartStreamingTimer() { startStreamingTimer.stop() }
    ...
    private func stopStopStreamingTimer() { stopStreamingTimer.stop() }
    ...
    func getState() -> DjiDeviceState { state }
}
...
extension DjiDevice: CBPeripheralDelegate {
    ...
    func peripheral(_: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error _: Error?) {
        ...
        switch state {
        case .checkingIfPaired:  processCheckingIfPaired(response: message)
        case .pairing:           processPairing()
        case .cleaningUp:        processCleaningUp(response: message)
        case .preparingStream:   processPreparingStream(response: message)
        case .settingUpWifi:     processSettingUpWifi(response: message)
        case .configuring:       processConfiguring(response: message)
        case .startingStream:    processStartingStream(response: message)
        case .streaming:         processStreaming(message: message)
        case .stoppingStream:    processStoppingStream(response: message)
        default:
    ...
    private func processCheckingIfPaired(response: DjiMessage) {
        guard response.id == pairTransactionId else { return }
    ...
    private func processCleaningUp(response: DjiMessage) {
        guard response.id == stopStreamingTransactionId else { return }
    ...
    private func processPreparingStream(response: DjiMessage) {
        guard response.id == preparingToLivestreamTransactionId,
              let wifiSsid, let wifiPassword else { return }
    ...
    private func processSettingUpWifi(response: DjiMessage) {
        guard response.id == setupWifiTransactionId else { return }
        ...
        case .osmoAction2, .osmoAction3:
            sendStartStreaming()
        case .osmoAction4:
            guard let imageStabilization else { return }
        ...
        case .osmoAction5Pro, .osmoAction6, .osmo360:
            ...
            writeMessage(message: DjiMessage(target: configureTarget,
                                             ...
                                             payload: payload.encode()))
            setState(state: .configuring)
        case .osmoPocket3, .unknown:
            sendStartStreaming()
        }
    }

    private func processConfiguring(response: DjiMessage) {
        guard response.id == configureTransactionId else { return }
        sendStartStreaming()
    }

    private func sendStartStreaming() {
        guard let rtmpUrl, let resolution else { return }
        ...
        writeMessage(message: DjiMessage(target: startStreamingTarget,
                                         ...
                                         payload: payload.encode()))
        // OA5 Pro / OA6 / Osmo 360 need a follow-up "confirm start" write that
        // mirrors the stop-stream packet with the last byte set to 0x01.
        if model.hasNewProtocol() {
            let confirm = DjiConfirmStartStreamingMessagePayload()
            writeMessage(message: DjiMessage(target: stopStreamingTarget,
                                             id: stopStreamingTransactionId,
                                             type: stopStreamingType,
    ...
    private func processStartingStream(response: DjiMessage) {
        guard response.id == startStreamingTransactionId else { return }
    ...
    private func processStreaming(message: DjiMessage) {
        switch message.type {
        case 0x020D00:
            guard message.payload.count >= 21 else { return }
            batteryPercentage = Int(message.payload[message.payload.startIndex + 20])
        default:
            break
        }
    }
    ...
    private func writeValue(value: Data) {
        guard let fff5Characteristic else { return }
    ...
    func peripheral(_: CBPeripheral,
        ...
        guard characteristic.uuid == fff4Id else { return }
        let payload = DjiPairMessagePayload(pairPinCode: pairPinCode)
        let request = DjiMessage(target: pairTarget,
