...
final class DjiPairMessagePayload {
...
final class DjiPreparingToLivestreamMessagePayload {
    static let payload = Data([0x1A])
    func encode() -> Data { DjiPreparingToLivestreamMessagePayload.payload }
}

final class DjiSetupWifiMessagePayload {
    var wifiSsid: String
...
final class DjiStartStreamingMessagePayload {
...
final class DjiConfirmStartStreamingMessagePayload {
    static let payload = Data([0x01, 0x01, 0x1A, 0x00, 0x01, 0x01])
    func encode() -> Data { DjiConfirmStartStreamingMessagePayload.payload }
}

final class DjiStopStreamingMessagePayload {
    static let payload = Data([0x01, 0x01, 0x1A, 0x00, 0x01, 0x02])
    func encode() -> Data { DjiStopStreamingMessagePayload.payload }
}

final class DjiConfigureMessagePayload {
    ...
    func encode() -> Data {
        let imageStabilizationByte: UInt8
        switch imageStabilization {
        case .off:              imageStabilizationByte = 0
        case .rockSteady:       imageStabilizationByte = 1
        case .rockSteadyPlus:   imageStabilizationByte = 3
        case .horizonBalancing: imageStabilizationByte = 4
        case .horizonSteady:    imageStabilizationByte = 2
        }
        let byte1: UInt8 = oa5 ? 0x1A : 0x08
        let writer = ByteWriter()
